...
async function fetchJson(path: string): Promise<unknown> {
  const response = await fetch(`/_local-data/v3/${path}`, { cache: 'no-store' });
...
async function loadPayloads(): Promise<MapPayload> {
  const [four, tarzana, airports, rail, mask] = await Promise.all([
...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadState('error');
      setError(cause instanceof Error ? cause.message : 'The local v3 payload could not be loaded.');
    }
  }, []);
  ...
  return (
    <main className={`app-shell ${selected.length ? 'inspect-open' : ''}`} aria-labelledby="product-name">
      ...
      <header className="brand-instrument glass" aria-label="Quiet LA identity">
        ...
        <span className="internal-badge">Private model workspace</span>
      ...
      <section className="noise-view-instrument glass" aria-label="Noise view">
        ...
        <p className="view-summary">{view === 'all' ? 'Visual co-display only · never an acoustic sum' : view === 'modeled' ? 'Modeled layers only · each result remains separate' : 'Official context and incomplete overlays only'}</p>
        <p className="headline-boundary">{contract.headlineBoundary}</p>
      ...
      <section className="period-instrument glass" aria-label="Scenario period">
        ...
        <div className="period-options" role="radiogroup" aria-label="Choose scenario period">
          {(['D', 'E', 'N'] as const).map((value) => (
            <button key={value} className={`period ${period === value ? 'is-active' : ''}`} type="button" role="radio" aria-checked={period === value} onClick={() => setPeriod(value)}><b>{value}</b><small>{value === 'D' ? 'Day' : value === 'E' ? 'Evening' : 'Night'}</small></button>
          ))}
        </div>
        <p className="period-note">Tarzana scenario changes with period. Four-region common-scale layer is unchanged.</p>
      </section>

      <details className="layer-instrument glass">
        <summary><span className="layer-instrument__icon" aria-hidden="true">≋</span><span><strong>Noise layers</strong><small>{mappedFamilyCount} mapped source families · {showRailContext ? '1 non-spatial context record' : 'context hidden'}</small></span></summary>
        <div className="layer-instrument__panel">
          ...
          <div className="layer-style-options" role="radiogroup" aria-label="Noise display style">
            {(['field', 'bands', 'dots', 'glow'] as const).map((style) => <button key={style} className={layerStyle === style ? 'is-active' : ''} type="button" role="radio" aria-checked={layerStyle === style} onClick={() => setLayerStyle(style)}><span className={`style-swatch style-swatch--${style}`} aria-hidden="true" />{style[0].toUpperCase() + style.slice(1)}</button>)}
          </div>
          <span className="layer-control-label">Source/status</span>
          <div className="layer-rows">
            {mappedLayers.map((layer) => (
              <label key={layer.id} className={`layer-row ${layerVisibleInView(layer, view) ? '' : 'is-filtered'}`}>
                <input type="checkbox" checked={layerToggles[layer.id] ?? false} onChange={() => toggleLayer(layer.id)} aria-label={`Toggle ${layer.label}`} />
                <span className="layer-swatch" aria-hidden="true" data-family={layer.family} />
                <span className="layer-row-copy"><strong>{layer.label}</strong><small>{classLabel(layer.evidenceClass)}</small></span>
              </label>
            ))}
            {showRailContext && railLayer && <div className="layer-row layer-row--context" data-testid="rail-context-row" aria-label="Metro rail metadata — non-spatial context"><span className="layer-swatch" aria-hidden="true" data-family="rail_context" /><span className="layer-row-copy"><strong>Metro rail metadata — non-spatial context</strong><small>{classLabel(railLayer.evidenceClass)}</small><em>6 active route metadata records; geometry and rail acoustics are not admitted.</em></span></div>}
      ...
      <section className="disclosure-rail glass" aria-label="Model disclosures">
        <div className="active-run"><span className="run-dot" aria-hidden="true" /><div><span className="run-label">Active local layers</span><strong>{mappedFamilyCount} mapped source families + {showRailContext ? 1 : 0} non-spatial context record · {receiverCount.toLocaleString()} receiver records</strong></div></div>
        <p><strong>{detailCount}</strong> nearest record{detailCount === 1 ? '' : 's'} per source family shown separately.</p>
        <ul><li>Modeled relative / uncalibrated and scenario assumed inputs.</li><li>Official records are context only; the mask is incomplete — not computable.</li><li>Not current traffic; not live traffic; not observed traffic; not measured dBA/CNEL; not quietness; not an address prediction.</li><li>Compatible acoustic sum is disabled until its exact contract is admitted.</li></ul>
        <button className="mobile-limits-button" type="button" onClick={() => setLimitsOpen(true)}>Model limits · open disclosures</button>
      </section>
      <p className="map-attribution"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap contributors</a> · Quiet LA modeled display</p>

      {loadState === 'loading' && <div className="load-state glass" role="status" aria-live="polite"><span className="spinner" aria-hidden="true" />Loading the accepted local v3 layers…</div>}
      {loadState === 'error' && <div className="load-state load-state--error glass" role="alert"><strong>Local data is not staged.</strong><span>{error}</span><button className="load-state__action" type="button" onClick={() => void load()}>Retry</button><small>Run <code>npm run stage:local</code> from this app for the local-only payloads. External builds refuse this tree.</small></div>}
      ...
      <dialog className="method-dialog" id="limits-dialog" open={limitsOpen} aria-labelledby="limits-title">
        ...
        <p><strong>Exact layers.</strong> The freeway field is modeled relative and uncalibrated. Tarzana is an assumed-input scenario. Airport and Metro records are official context only. Source-341 is explicitly incomplete — not computable.</p>
